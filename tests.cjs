...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:19,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
{
const g=harness();g.start('local');g.bossWave(6);g.spawnBoss();const [p,q]=g.state().players,e=g.state().enemies.at(-1);p.hp=p.maxHp;q.hp=q.maxHp;assert(!g.bossRelief());assert(Math.abs(g.turnAim(0,Math.PI,.1)-.12)<1e-9,'slower boss aim');assert(Math.abs(g.turnAim(0,.01,.1)-.01)<1e-9,'no overshoot');assert(g.turnAim(3.13,-3.13,.01)>3.13,'shortest angle across seam');

...
bossTest.start('solo');bossTest.bossWave(2);bossTest.simulate(.016);let boss=bossTest.state().enemies.find(e=>e.type==='boss');assert(boss);assert.equal(bossTest.state().between,false);const target=bossTest.state().players[0],soloHP=boss.hp;
...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:13,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
const remembered=rp.rivalHabits.samples;rival.windup=1;
...
assert.equal(copy.sourceId,0);assert.equal(copy.weapons[0].tier,4);assert.equal(copy.speed,400);assert.equal(copy.dashTime,.8);assert(Object.values(copy.abilities).every(Boolean));assert(copy.nominalDps>combined);assert((copy.hp+copy.shield)*(1+copy.armor*.08)>copy.teamEhp);
...
mirror.mirrorCombat(copy,.36);mirror.mirrorCombat(copy,.36);const hpBefore=m0.hp,aimBefore=copy.weapons[0].lockedAim;assert(copy.weapons[0].pending>0);
m0.x+=100;mirror.mirrorCombat(copy,.1);assert.equal(copy.weapons[0].lockedAim,aimBefore);assert.equal(m0.hp,hpBefore);
...
console.log('PASS v1.13: strongest build, team ability union, combined power, isolated inventory, warned laser, all hostile weapons, late pressure, larger sphere and mirror restore/render.');
...
e.brain.target={id:p.id,x:p.x,y:p.y,vx:0,vy:0,habits:{samples:20,speed:150,range:200,orbit:0}};e.brain.scan=1;e.brain.dashRequest=true;const before={x:e.x,y:e.y};g.mirrorCombat(e,.01);assert(g.distance(before,e)>100,'real dash displacement');assert(e.evade>0);assert(e.novaWarning>0);const ehp=e.hp;g.hurt(e,500);assert.equal(e.hp,ehp,'brief dash immunity');

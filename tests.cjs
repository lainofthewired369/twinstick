...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:24,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
const treePilot=make();P.grant(treePilot,0,100);assert.equal(treePilot.skillPoints,treePilot.level-1);assert.equal(P.skills.nodes.length,52);
...
{
 const g=harness(),events=[],packets=[];g.host();for(let id=1;id<8;id++){const h={};events[id]=h;g.wire({open:true,bufferSize:0,on:(n,f)=>h[n]=f,send:m=>{if(id===1&&m.t==='state')packets.push(JSON.parse(JSON.stringify(m)));}},0);h.data({t:'ready',v:24,token:'inventory-test-000'+id,config:{starter:id%2?'smg':'pistol'}});}g.start('online');g.openShop();
...
const T3=require('./third-person.js');
for(const heading of [0,.6,-Math.PI/2,Math.PI]){const focus={x:70,y:-120,cameraAngle:heading};for(const across of [-150,0,150]){const q={x:focus.x+Math.cos(heading)*600-Math.sin(heading)*across,y:focus.y+Math.sin(heading)*600+Math.cos(heading)*across},screen=T3.project(q,focus),round=T3.unproject(screen.x,screen.y,focus);assert(screen.visible);assert(Math.hypot(round.x-q.x,round.y-q.y)<1e-6);}}
const cover=T3.pylons[0],atCover=T3.resolve({x:cover.x,y:cover.y},15);assert(Math.hypot(atCover.x-cover.x,atCover.y-cover.y)>=cover.r+15-.001);assert(T3.blocked({x:cover.x-200,y:cover.y},{x:cover.x+200,y:cover.y},3));assert(!T3.blocked({x:0,y:0},{x:0,y:-200}));
assert(T3.clip([{x:0,y:0,z:0},{x:1,y:0,z:30},{x:0,y:1,z:30}]).every(p=>p.z>=T3.NEAR));
const ascension=harness();ascension.start('local');ascension.setTier(3);ascension.bossWave(20);ascension.simulate(.01);const lastOrbitalBoss=ascension.state().enemies.find(e=>e.type==='boss');ascension.hurt(lastOrbitalBoss,1e12);assert.equal(ascension.state().fractureLeft,3);assert.equal(ascension.state().visualTier,3);ascension.update(1.5);assert.equal(ascension.state().visualTier,4);assert(ascension.state().players.every(p=>[p.x,p.y,p.cameraAngle].every(Number.isFinite)));ascension.update(1.6);assert(ascension.state().between);const thirdPilot=ascension.state().players[0];thirdPilot.x=0;thirdPilot.y=0;thirdPilot.cameraAngle=-Math.PI/2;ascension.move(thirdPilot,{dx:0,dy:-1,angle:-Math.PI/2,fire:false,dash:false},.1);assert(thirdPilot.y<0);assert(Math.abs(thirdPilot.x)<1e-6);const cam=thirdPilot.cameraAngle;ascension.move(thirdPilot,{dx:0,dy:0,angle:0,fire:false,dash:false},.1);assert(thirdPilot.cameraAngle>cam);assert(thirdPilot.cameraAngle-cam<=.151);
ascension.breakRift();assert.equal(ascension.state().fractureLeft,0);ascension.start('solo');assert.equal(ascension.state().visualTier,0);
softVM.window.RRThird=T3;softVM.window.RRProgress=P;
assert(softVM.window.Rift3D.draw(planetFrame,{third:true,focus:{x:0,y:0,cameraAngle:-Math.PI/2},t:1,reduced:true,players:[{id:0,x:0,y:0,angle:-Math.PI/2,color:'#68f7c2',hp:100,maxHp:100,weapons:[P.weapon('pistol')]}],enemies:[],shots:[],enemyShots:[],drops:[],effects:[],particles:[]}));
const thirdNet=require('./network-visuals.js').create(S,T3);const netPilot={id:0,x:-500,y:-500,hp:100,speed:250,angle:-Math.PI/2,cameraAngle:-Math.PI/2};thirdNet.receive({visualTier:4,wave:21,running:true,players:[netPilot],enemies:[],shots:[],enemyShots:[]},1000);thirdNet.predict(netPilot,{dx:0,dy:-1,angle:-Math.PI/2},.016,1016,true);const predictedThird=thirdNet.render({players:[netPilot],enemies:[],shots:[],enemyShots:[]},1016,0).players[0];assert(predictedThird.x<0&&predictedThird.y<-500);
console.log('PASS wave 20: boss-triggered transition, projection round trips, solid pylons, near-plane clipping, camera-relative controls, phase reset, software renderer and third-person guest prediction.');

for(const tier of [0,1.5,3,4]){
 const g=harness();g.start('solo');g.setTier(tier);g.bossWave(12);const p=g.state().players[0];p.x=500;p.y=350;
 for(const type of ['skirmisher','bomber','medic'])g.spawnVariant(type,700,350);
 const [sk,bomb,med]=g.state().enemies;sk.attack=0;g.enemyAttack(sk,p,.016);assert.equal(g.state().enemyShots.length,0);const committed=sk.aim;p.y+=40;g.enemyAttack(sk,p,.7);assert.equal(sk.aim,committed);assert.equal(g.state().enemyShots.length,3);
 g.setBullets([]);bomb.attack=0;g.enemyAttack(bomb,p,.01);assert.equal(g.state().enemyShots.length,0);g.enemyAttack(bomb,p,1.01);assert.equal(g.state().enemyShots.length,9);assert(g.state().enemyShots.every(s=>[s.x,s.y,s.vx,s.vy,s.damage].every(Number.isFinite)));
 sk.hp=sk.maxHp*.5;med.attack=0;g.enemyAttack(med,p,.01);assert(sk.hp>sk.maxHp*.5&&sk.hp<=sk.maxHp);const hp=sk.hp;g.enemyAttack(med,p,.01);assert.equal(sk.hp,hp);
 g.setBullets([]);for(const e of [sk,bomb,med]){e.attack=99;e.windup=0;}
 const before={x:sk.x,y:sk.y};g.simulate(.016);assert(g.distance(before,sk)>0);assert(g.state().enemies.every(e=>[e.x,e.y,e.hp].every(Number.isFinite)));
}
const leadGame=harness();leadGame.start('solo');leadGame.spawnVariant('skirmisher',100,350);const leader=leadGame.state().enemies[0],leadTarget=leadGame.state().players[0];leadTarget.x=500;leadTarget.y=350;leader.attack=99;leadGame.enemyAttack(leader,leadTarget,.1);leadTarget.y+=20;leader.attack=0;leadGame.enemyAttack(leader,leadTarget,.1);assert(leader.aim>Math.atan2(20,400));
for(const type of ['skirmisher','bomber','medic']){const model=require('./rift-models.js').get(type);assert(model.vertices.length>0);}
console.log('PASS v1.26: new enemy windups, committed volleys, bomber safe gap, capped repair pulses, movement across all worlds, leading shots and model generation.');

// Expanded tree remains connected and keeps the original abilities reachable.
assert.equal(new Set(P.skills.nodes.map(n=>n.id)).size,52);
const treeReach=new Set(['core']);for(let i=0;i<52;i++)for(const n of P.skills.nodes)if(n.requires.some(id=>treeReach.has(id)))treeReach.add(n.id);assert.equal(treeReach.size,52);
const caps=['chainLightning','freezeRay','gravityWell','aegisPulse','clusterBarrage'];
for(const cap of caps){const pilot=make();pilot.skillPoints=100;const n=P.skills.nodes.find(n=>n.ability===cap);assert(n);assert(!P.skills.available(pilot,n.id));let path=[],cursor=n;while(cursor.id!=='core'){path.unshift(cursor.id);cursor=P.skills.byId[cursor.requires[0]];}for(const id of path)assert(P.skills.unlock(pilot,id,()=>{}));assert(pilot.abilities[cap]);assert(!P.skills.unlock(pilot,n.id,()=>{}));}
for(const tier of [0,1.5,3,4])for(const cap of caps){
 const g=harness();g.start('solo');g.setTier(tier);const p=g.state().players[0];p.x=500;p.y=350;p.angle=0;p.abilities={[cap]:true};p.abilityTimers={};
 for(let n=0;n<6;n++)g.spawnVariant('tank',580+n*30,350);for(const e of g.state().enemies)e.hp=e.maxHp=10000;
 g.setBullets([{x:p.x+20,y:p.y,r:5,life:3,vx:1,vy:0,damage:5},{x:p.x+400,y:p.y,r:5,life:3,vx:1,vy:0,damage:5}]);
 g.tickAbilities(p,.016);const state=g.state();assert(p.abilityTimers[cap]>0);
 if(cap==='chainLightning')assert.equal(state.enemies.filter(e=>e.hp<10000).length,5);
 if(cap==='freezeRay'){assert(state.enemies[0].hp<10000);assert(state.enemies[0].stun>0);assert.equal(state.enemies[0].slow,.65);}
 if(cap==='gravityWell'){assert(state.enemies[1].hp<10000);assert(state.enemies[1].x!==610||state.enemies[1].y!==350);}
 if(cap==='aegisPulse'){assert.equal(state.enemyShots.length,1);assert.equal(p.invuln,.6);}
 if(cap==='clusterBarrage'){assert.equal(state.shots.length,5);assert(state.shots.every(s=>s.owner===p.id&&s.blast>0));}
 const damage=state.enemies.reduce((sum,e)=>sum+e.hp,0),shots=state.shots.length;g.tickAbilities(p,.01);assert.equal(g.state().enemies.reduce((sum,e)=>sum+e.hp,0),damage);assert.equal(g.state().shots.length,shots);
 assert(state.enemies.every(e=>[e.x,e.y,e.hp].every(Number.isFinite)));
}
console.log('PASS v1.27: 52 connected nodes, gated capstone purchases, five real abilities, cooldowns, ownership and finite behavior across flat, frontier, sphere and third-person worlds.');

...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:15,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
const labels=harness();labels.restoreState(JSON.parse(JSON.stringify(n.state())));labels.drawDamageNumbers();assert(labels.numberDraws.some(x=>x.label==='P1 10'));assert(labels.numberDraws.some(x=>x.label==='P2 20'));assert.notEqual(labels.numberDraws[0].y,labels.numberDraws[1].y,'simultaneous teammate labels do not overlap');

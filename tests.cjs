...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:17,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
for(let wave=2;wave<=8;wave++){g.start('solo');g.bossWave(wave);g.simulate(.01);const b=g.state().enemies.find(e=>e.type==='boss');assert(b,'boss on wave '+wave);assert.equal(!!b.mirror,wave>=6);if(wave>=6){assert(b.brain);assert(b.weapons.length>0);}g.simulate(.01);assert.equal(g.state().enemies.filter(e=>e.type==='boss').length,1);}
...
for(const wave of [3,7,10,15,16,20]){
 ...
 assert(Math.abs(e.maxHp-hp*(reduced?.8:1))<1e-6);assert.equal(e.hp,e.maxHp);assert.equal(e.maxShield,40*(reduced?.8:1));assert.equal(e.shield,e.maxShield);
 assert(Math.abs(e.nominalDps-power.dps*base*(reduced?.85:1))<1e-6);assert(Math.abs(e.damage-p.damage*power.dps/g.combatPower(p,true).dps*base*(reduced?.85:1))<1e-6);assert.equal(e.healBudget,e.maxHp*.2);
}
console.log('PASS v1.17.1: waves 3–15 have 20% less HP/shield and 15% less build damage; wave 16+ retains original scaling.');
}

{
const g=harness();g.start('local');const [p,q]=g.state().players;assert.equal(p.lives,3);p.x=400;p.y=300;q.x=450;q.y=300;p.shield=0;p.armor=0;g.damagePlayer(p,1000);assert.equal(p.lives,2);assert(p.downed);g.damagePlayer(p,1000);assert.equal(p.lives,2,'dead ship cannot lose extra lives');
g.tickRevives(4);assert.equal(p.reviveProgress,4);q.x=501;g.tickRevives(10);assert.equal(p.reviveProgress,4,'out of range pauses total');q.x=500;g.tickRevives(5.9);assert.equal(p.hp,0);g.tickRevives(.11);assert.equal(p.hp,p.maxHp*.5);assert.equal(p.invuln,3);assert(!p.downed);assert.equal(p.lives,2);
g.damagePlayer(p,1000);assert.equal(p.lives,1);g.tickRevives(10);assert(p.hp>0);g.damagePlayer(p,1000);assert.equal(p.lives,0);assert(!p.downed);g.tickRevives(20);assert.equal(p.hp,0,'third defeat is final');P.open(p,3,['pistol']);P.apply(p,'health');assert.equal(p.hp,0,'shop cannot bypass elimination');
const s=harness();s.start('solo');const pilot=s.state().players[0];s.damagePlayer(pilot,1000);s.simulate(9);assert(s.state().running);assert.equal(pilot.hp,0);s.simulate(1);assert(pilot.hp>0,'solo uses same ten second respawn');
const team=harness();team.start('local');for(const ally of team.state().players)team.damagePlayer(ally,10000);team.simulate(.01);assert(!team.state().running,'team wipe ends co-op run');
const sphere=harness();sphere.start('local');sphere.setTier(3);const [a,b]=sphere.state().players;a.x=1278;a.y=360;b.x=2;b.y=360;sphere.damagePlayer(a,1000);sphere.tickRevives(3);assert.equal(a.reviveProgress,3,'sphere seam uses physical distance');b.connected=false;sphere.tickRevives(5);assert.equal(a.reviveProgress,3,'disconnected helpers do not count');
const copy=harness();copy.restoreState(JSON.parse(JSON.stringify(sphere.state())));const restored=copy.state().players[0];assert.equal(restored.reviveProgress,3);assert.equal(restored.lives,2);assert(restored.downed);g.start('solo');assert.equal(g.state().players[0].lives,3);
console.log('PASS v1.18: three lives, cumulative 10-second proximity revive, range boundary, final elimination, shop protection, solo respawn, team wipe, sphere seam and checkpoint restoration.');
}
{
for(const wave of [1,2,3,5,6,15,16]){const g=harness();g.start('solo');g.bossWave(wave);g.spawnBoss();const b=g.state().enemies.at(-1);assert.equal(!!b.mirror,wave>=6);assert.equal(!!b.rival,wave>=6);if(wave<=5){assert(!b.brain);assert(!b.weapons);const expected=(220+wave*100)*b.power.health*g.state().enemyScale.health*(wave>=3?.8:1);assert(Math.abs(b.maxHp-expected)<1e-6);assert(Math.abs(b.power.damage-g.bossPower().damage*(wave>=3?.85:1))<1e-6);}}
console.log('PASS v1.18 boss schedule: ordinary Wardens through wave 5, learning mirrors from wave 6, wave 3–5 relief and original wave 1–2 balance.');
}

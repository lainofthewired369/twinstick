...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:22,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
{
 const g=harness(),events=[],packets=[];g.host();for(let id=1;id<8;id++){const h={};events[id]=h;g.wire({open:true,bufferSize:0,on:(n,f)=>h[n]=f,send:m=>{if(id===1&&m.t==='state')packets.push(JSON.parse(JSON.stringify(m)));}},0);h.data({t:'ready',v:22,token:'inventory-test-000'+id,config:{starter:id%2?'smg':'pistol'}});}g.start('online');g.openShop();

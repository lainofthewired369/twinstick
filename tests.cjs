...
const sharedHost=harness(),sharedEvents={},sharedPackets=[];sharedHost.host();sharedHost.wire({open:true,bufferSize:0,on:(n,f)=>sharedEvents[n]=f,send:m=>sharedPackets.push(JSON.parse(JSON.stringify(m)))},0);sharedEvents.data({t:'ready',v:18,token:'test-token-000000',config:{starter:'pistol'}});sharedHost.start('online');
...
assert.equal(copy.sourceId,0);assert.equal(copy.weapons[0].tier,4);assert.equal(copy.speed,280);assert.equal(copy.dashTime,30);assert(Object.values(copy.abilities).every(Boolean));assert(copy.nominalDps>combined);assert((copy.hp+copy.shield)*(1+copy.armor*.08)>copy.teamEhp);
...
for(const wave of [2,6,16]){const boss=harness();boss.start('solo');boss.bossWave(wave);boss.spawnBoss();const e=boss.state().enemies.at(-1),expected=(wave>=6?boss.state().players[0].speed:48+Math.min(30,wave))*.7;assert.equal(e.speed,expected);e.x=300;e.y=300;e.dash=0;boss.startRam(e,0);boss.tickRam(e,.1);assert(Math.abs(e.x-358.5)<.001,'boss ram is 35% slower');}

...
(async()=>{
 try{
  ...
  assert.equal(guest.state().players[1].materials,beforeMoney-lifePrice);assert.equal(host.state().players[0].lives,10);console.log('PASS life shop: host calculates price and credits only requesting player');
  ...
  assert.equal(guest.state().players[1].lives,11);rescuer.x=casualty.x+150;host.tickRevives(6);assert.equal(casualty.reviveProgress,4);rescuer.x=casualty.x+50;host.tickRevives(6);host.sendState(true);
  await until(()=>!guest.state().players[1].downed&&guest.state().players[1].hp>0,'proximity revive');
  assert.equal(guest.state().players[1].lives,11);console.log('PASS revival: lives, paused cumulative timer and completed revive cross real RTC');
  ...
  await until(()=>guest.state().players[1].hp>0&&guest.state().players[1].lives===10,'instant guest respawn');

...
function build(type,low=false){
 ...
 if(type==='skirmisher'||type==='ship'||type==='runner'||type==='charger'||type==='gunner'||type==='swarm'){
 ...
 }else if(type==='bomber'||type==='tank'||type==='sentinel'||type==='boss'||type==='lancer'){
  ...
  if(type==='medic'){tube([[-18,3,3],[18,3,3]],0,29,'engine');tube([[-3,18,3],[3,18,3]],0,29,'engine');}if(!low)engine(0);
 }

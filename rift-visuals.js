...
function fracture(x,left,reduced=false,W=1280,H=720,target=1){if(left<=0)return;if(target===1.5)window.RiftFrontier?.walls(x,left,reduced,W,H);const age=3-left,cx=W*.5,cy=H*.5;x.save();
 ...
 x.globalAlpha=Math.min(1,age*3,left*2);x.fillStyle='#081224ed';x.fillRect(cx-320,cy-48,640,98);x.textAlign='center';x.fillStyle='#b4f7ff';x.font='700 32px system-ui';x.fillText(age<1.4?(target===4?'HORIZON UNBOUND':target===3?'WORLD REFORGED':target===2?'DIMENSIONS FRACTURED':target===1.5?'CONTAINMENT SHATTERED':'REALITY FRACTURED'):(target===4?'LAYER VI // THIRD PERSON':target===3?'LAYER V // PLANET ONLINE':target===2?'LAYER IV // ENTERING 3D':target===1.5?'LAYER III // ENDLESS FRONTIER':'RIFT LAYER II // UNLOCKED'),cx,cy-4);x.font='13px system-ui';x.fillStyle='#aec3de';x.fillText(target===4?'CHASE CAMERA · 3D ARENA · KEEP MOVING':target===3?'NO EDGES · FOLLOW THE CURVE · KEEP MOVING':target===2?'DEPTH RECONSTRUCTED · 3D WORLD ONLINE':target===1.5?'WALLS DESTROYED · EXPLORE IN EVERY DIRECTION':'SIGNAL RECONSTRUCTED · VISUAL SYSTEMS EVOLVED',cx,cy+24);x.restore();
}

...
function player(x,color,id){return{x,y:360,angle:0,hp:100,maxHp:100,color,id,speed:250,damage:22,rate:.18,cool:0,dash:0,dashTime:1.5,multi:1};}
function show(id){overlay=id;['menu','lobby','upgrade','gameover'].forEach(n=>$('#'+n).classList.toggle('hidden',n!==id));}
function resetInput(){
...
function start(m){
 resetInput();mode=m;wave=0;score=0;enemies=[];shots=[];particles=[];choices=[];
...
function spawn(){const side=Math.floor(Math.random()*4),tank=Math.random()<Math.min(.12+wave*.02,.42);enemies.push({x:side%2?Math.random()*W:side?W+30:-30,y:side%2?(side===1?-30:H+30):Math.random()*H,r:tank?22:14,hp:tank?80+wave*12:35+wave*6,speed:tank?55:Math.min(240,90+wave*3),type:tank?'tank':'drone',hit:0});}
...
function shoot(p){if(p.cool>0||p.hp<=0||shots.length>600)return;p.cool=p.rate;for(let i=0;i<p.multi;i++){const a=p.angle+(i-(p.multi-1)/2)*.13;shots.push({x:p.x+Math.cos(a)*18,y:p.y+Math.sin(a)*18,vx:Math.cos(a)*650,vy:Math.sin(a)*650,life:1.2,dmg:p.damage,owner:p.id});}}
...
function move(p,i,dt){
 if(p.hp<=0)return;
 p.cool=Math.max(0,p.cool-dt);p.dash=Math.max(0,p.dash-dt);p.angle=i.angle;
...
function simulate(dt){
 ...
 for(const s of shots){
  s.x+=s.vx*dt;s.y+=s.vy*dt;s.life-=dt;if(s.life<=0)continue;
  for(const e of enemies)if(e.hp>0&&Math.hypot(s.x-e.x,s.y-e.y)<e.r+4){
   e.hp-=s.dmg;s.life=0;e.hit=.1;if(e.hp<=0){score+=e.type==='tank'?50:20;burst(e.x,e.y,e.type==='tank'?'#ffc55c':'#ff496c',14);}break;
  }
 }
...
function ui(){
 ...
 const p=players[localId];$('#dashTouch').textContent=p?.dash>0?p.dash.toFixed(1)+'s':'DASH';
 if(overlay==='menu'||overlay==='lobby')return;
...
function sendState(){if(isHost&&mode==='online')send({t:'state',v:2,players,enemies,shots,wave,score,spawnLeft,running,paused,between,choices});}
...
async function roomId(password){
 const hash=await crypto.subtle.digest('SHA-256',new TextEncoder().encode('rift-runners-v2:'+password));
 return 'rr2-'+Array.from(new Uint8Array(hash),b=>b.toString(16).padStart(2,'0')).join('');
}
async function connect(host){
 ...
 try{
  ...
  p.on('open',()=>{
   ...
   else {netStatus('Connecting to host…');wire(p.connect(id,{reliable:true,serialization:'json',metadata:{v:2}}),token);}
  });
  p.on('connection',c=>{
...
function wire(c,token){
 ...
 c.on('open',()=>{if(token!==session)return;lastPacket=performance.now();if(!isHost)send({t:'ready',v:2});});
 c.on('data',m=>{
  if(token!==session||!m||typeof m!=='object')return;lastPacket=performance.now();
  if(m.t==='reject'){fail(m.reason||'Room unavailable.');return;}
  if(isHost&&m.t==='ready'&&m.v===2){clearTimeout(netTimer);mode='online';start('online');return;}
  if(isHost&&m.t==='input'){
  ...
  if(!isHost&&m.t==='state'&&m.v===2){
   if(!Array.isArray(m.players)||m.players.length!==2||!Array.isArray(m.enemies)||!Array.isArray(m.shots))return;
   clearTimeout(netTimer);mode='online';players=m.players;enemies=m.enemies;shots=m.shots;wave=m.wave;score=m.score;spawnLeft=m.spawnLeft;running=m.running;paused=m.paused;between=m.between;choices=m.choices;

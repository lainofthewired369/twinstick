...
function wire(c,token){
 ...
 c.on('data',m=>{
  ...
  if(accepting){
   if(!isHost)return;
   if(m.t==='ready'&&!link){
    if(m.v!==21||typeof m.token!=='string'||m.token.length<16||m.token.length>128){rejectConnection(c,'Refresh to v1.24.0 and join again.');return;}

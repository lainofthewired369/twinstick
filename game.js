...
function playCombat(e,silent=false){const gain=silent?0:Math.max(0,1-distance(cameraFocus(),e)/1100);const v=delta(cameraFocus(),e),a=controlAngle(cameraFocus());feedback?.event(e,gain,!!reducedMotion?.matches,{x:v.x*Math.cos(a)+v.y*Math.sin(a),y:-v.x*Math.sin(a)+v.y*Math.cos(a)});}
...
function soundButton(){if(!feedback)return;const b=$('#soundBtn');b.textContent=feedback.isMuted()?'♪ OFF':'♪ '+['I','II','III','IV','V'][feedback.stage];b.setAttribute('title','Audio '+(feedback.stage+1)+' · '+feedback.name);b.setAttribute('aria-label',feedback.isMuted()?'Enable sound':'Mute sound');b.setAttribute('aria-pressed',String(!feedback.isMuted()));}
...
const netVisual=window.RRNetworkVisuals?.create(Sphere,window.RRThird);
let uiClock=0;
...
const Third=window.RRThird;
const inThird=()=>visualTier===4;
const onSphere=()=>visualTier===3;
const openField=()=>visualTier===1.5||visualTier===2||inThird();
...
function switchVisualTier(target){
 ...
 if(target===4&&visualTier===3&&(mode!=='online'||isHost)){
  const origin={...cameraFocus()};
...
let drops=[],shopPlayer=0,profile={best:0,reached:0,characters:{}},saveWarning='',lastSaved='';
try{const saved=JSON.parse(localStorage.getItem('rift-profile-v1')||'{}');if(Number.isFinite(saved.best))profile.best=Math.max(0,saved.best);profile.reached=Math.max(profile.best,Number.isFinite(saved.reached)?saved.reached:profile.best);if(saved.characters&&typeof saved.characters==='object')profile.characters=saved.characters;}catch(e){saveWarning='Progress saving is unavailable in this browser.';}
function recordProgress(){
 profile.reached=Math.max(profile.reached||0,wave);
...
function armory(){
 $('#armoryTitle').textContent=armoryPlayer?'Choose player 2’s ship':'Choose your ship';
 $('[data-action="armory-done"]').textContent=startChoice?'LAUNCH →':'← DONE';
 ...
 for(const [id,c] of Object.entries(P.characters)){const locked=(c.reach?profile.reached:profile.best)<c.unlock,b=document.createElement('button'),best=profile.characters[id];b.innerHTML='<svg class=ship-preview viewBox="-40 -35 80 70" aria-hidden="true"><polygon points="'+P.hulls[id].map(q=>q.join(',')).join(' ')+'"/><path d="M12 0 L-5 -5 L-5 5 Z"/></svg><b>'+c.name+(locked?' · '+(c.reach?'Reach':'Clear')+' wave '+c.unlock:'')+'</b><span>'+c.desc+'</span><small>Best wave '+(best?.best||0)+' · Best level '+(best?.level||1)+'</small>';b.disabled=locked;b.className=configs[armoryPlayer].character===id?'selected':'';b.onclick=()=>{configs[armoryPlayer].character=id;armory();};$('#slots').append(b);}
...
function drawDamageNumbers(){
 ...
 for(const n of damageNumbers){
  const point=inThird()?Third.project(n,cameraFocus(),25):onSphere()?Sphere.project(n,cameraFocus(),25):{x:n.x+(visualTier>=2?9:0),y:n.y-(visualTier>=2?13:0)};
...
function start(m){
 ...
 resetInput();feedback?.reset();soundButton();combatEvents=[];combatSerial=0;combatRates.clear();worldSeed=(Math.random()*2147483647)|0;visualTier=0;fractureLeft=0;riftBroken=false;fractureTarget=0;mode=m;wave=0;score=0;enemies=[];shots=[];particles=[];effects=[];damageNumbers=[];damageSerial=0;enemySerial=0;choices=[];drops=[];
...
function spawnBoss(){
 bossSpawned=true;const mirrorWave=wave>=10&&wave%5===0,power=bossPower(),hp=(!mirrorWave?220+wave*100:1000+wave*150)*(1+.8*Math.max(0,activePlayers().length-1))*power.health*enemyScale.health*latePressure();
 enemies.push({id:++enemySerial,type:'boss',rival:mirrorWave,brain:mirrorWave?{clock:0,scan:0,seen:{},target:null,turn:1,dodge:0}:null,power,reinforce:7,x:W/2,y:90,r:42,hp,maxHp:hp,speed:48+Math.min(30,wave),hit:0,burn:0,burnDamage:0,attack:1.8,windup:0,pattern:0,aim:0});
 ...
 if(mirrorWave)buildMirror(boss);else if(wave>=3&&wave<=15){boss.hp*=.8;boss.maxHp*=.8;boss.power.damage*=.85;boss.damage=22*.85;}
 boss.speed*=.7;
...
function buildMirror(e){
 ...
 e.bossCycle={step:0,kind:'FAN',phase:'warning',time:1,duration:1,weaponIndex:0};
...
function mirrorFire(e,slot,angle){
 ...
 if(w.reach){
  ...
  for(const p of activePlayers()){const d=distance(e,p),a=aimAt(e,p)-angle,hit=w.arc?d<reach+15&&Math.abs(Math.atan2(Math.sin(a),Math.cos(a)))<w.arc/2+15/Math.max(15,d):distanceToSegment(p.x,p.y,e.x,e.y,end.x,end.y)<20;if(hit&&(!inThird()||!Third.blocked(e,p)))mirrorHit(e,p,damage*(1+(e.multi-1)*.15));}
 }else{
...
function mirrorPattern(e,target,dt){
 const c=e.bossCycle||(e.bossCycle={step:0,kind:'FAN',phase:'warning',time:1,duration:1,weaponIndex:0});
 c.time=Math.max(0,c.time-dt*(e.mercy?.8:1));
 if(c.time===0){
  if(c.phase==='warning'){c.phase='attack';c.time=c.duration=c.kind==='SWEEP'?1.5:c.kind==='RAM'?1:1.1;c.fired=false;}
  else if(c.phase==='attack'){c.phase='recover';c.time=c.duration=e.mercy?2.1:1.65;for(const slot of e.weapons)slot.pending=0;e.novaWarning=0;}
  else{c.step++;c.kind=['FAN','SWEEP','RING','RAM'][c.step%4];if(c.kind==='RAM'&&e.dash>0)c.kind='FAN';c.phase='warning';c.time=c.duration=1;c.weaponIndex=c.step%e.weapons.length;}
 }
 if(c.phase==='warning'){e.windup=Math.max(e.windup,c.time);return false;}
 if(c.phase==='recover'){e.windup=0;return false;}
 const progress=1-c.time/c.duration;e.patternAim=e.aim+(c.kind==='SWEEP'?(progress-.5)*1.1:c.kind==='FAN'?[-.25,0,.25][Math.floor(progress*6)%3]:0);
 if(c.kind==='RING'&&!c.fired){c.fired=true;const gap=e.aim+(e.brain?.target?.habits?.orbit||0)*.4;for(let n=2;n<14;n++)mirrorProjectile(e,gap+n*Math.PI/7,{speed:225,life:3},e.damage*.55);}
 if(c.kind==='RAM'&&!c.fired){c.fired=true;startRam(e,e.aim);}
 return true;
}
function mirrorCombat(e,dt,patterned=false){
 ...
 const target=rivalThink(e,dt);bossRam(e,patterned?null:target,dt);healMirror(e,e.regen*dt);e.shieldDelay=Math.max(0,(e.shieldDelay||0)-dt);if(!e.shieldDelay){const recharge=Math.min(e.healBudget,e.maxShield-e.shield,e.maxShield*.15*dt);e.shield+=recharge;e.healBudget-=recharge;}if(!target||e.attack>0)return;
 e.aim=turnAim(e.aim,aimAt(e,target),dt);
 if(patterned&&!mirrorPattern(e,target,dt))return;
 for(const [index,slot] of e.weapons.entries()){
  if(patterned&&(index!==e.bossCycle.weaponIndex||['RING','RAM'].includes(e.bossCycle.kind)))continue;
  ...
  if(w.reach){slot.pending=w.arc?.55:.9;slot.shotAim=e.aim;slot.cool=Math.max(rate,slot.pending+.12);}else {mirrorFire(e,slot,patterned?e.patternAim:e.aim);slot.cool=Math.max(.06,rate);}
 }
...
function rivalThink(e,dt){
 ...
 if(!e.ramLeft&&(e.mirror||(!e.windup&&!e.beamLeft))){
  ...
  const boost=b.dodge>2.15?1.8:1,speed=(e.mirror?e.speed:115+Math.min(45,wave*2))*(e.mirror?1:boost)*(e.mirror&&bossRelief()?.8:1)*(e.bossCycle?.phase==='recover'?.35:1)*(e.slowTime>0?1-e.slow:1);
...
function bossAttack(e,target,dt){
 if(e.mirror){mirrorCombat(e,dt,true);return;}
...
const enemyTypes={
 skirmisher:{color:'#ffb347',r:13,hp:58,speed:125},bomber:{color:'#ff72be',r:25,hp:145,speed:58},medic:{color:'#49f5cd',r:19,hp:90,speed:82},
...
function tickBeam(e,dt){
 ...
 for(const p of activePlayers()){const d=delta(e,p),along=d.x*Math.cos(e.aim)+d.y*Math.sin(e.aim),across=Math.abs(d.y*Math.cos(e.aim)-d.x*Math.sin(e.aim));if(p.hp>0&&(!inThird()||!Third.blocked(e,p))&&along>0&&along<length&&across<21&&!p.invuln&&p.dash<p.dashTime-.15)damagePlayer(p,(40+wave*2)*enemyScale.damage*waveDamage()*dt,e);}
}
...
function spawn(){const side=Math.floor(Math.random()*4),pool=wave<2?['drone','drone','tank']:wave<4?['drone','tank','runner','gunner']:['drone','tank','runner','gunner','charger','splitter','sentinel'],extra=wave>=8?['skirmisher','bomber','medic']:wave>=6?['skirmisher','bomber']:wave>=3?['skirmisher']:[],roster=pool.concat(extra,extra),type=wave>=10&&Math.random()<(wave>=15?.28:.16)?'lancer':roster[Math.floor(Math.random()*roster.length)];if(openField()){const at=fieldSpawn();spawnVariant(type,at.x,at.y);}else if(onSphere()){const at=offset(activePlayers().find(p=>p.hp>0)||{x:640,y:360},Math.random()*Math.PI*2,300);spawnVariant(type,at.x,at.y);}else spawnVariant(type,side%2?Math.random()*W:side?W+30:-30,side%2?(side===1?-30:H+30):Math.random()*H);}
// Role steering is constant-cost per enemy; support scans only on its pulse.
function enemySteer(e,t,dt){
 const a=aimAt(e,t),d=distance(e,t),side=e.id%2?1:-1;
 let radial=1,strafe=0;
 if(['gunner','sentinel','lancer','skirmisher','bomber','medic'].includes(e.type)){
  const range=e.type==='medic'?350:e.type==='bomber'?230:e.type==='skirmisher'?300:280;
  radial=d>range+70?1:d<range-65?-.7:0;
  strafe=(e.type==='skirmisher'?.85:e.type==='medic'?.45:.3)*side;
  if(inThird()&&Third.blocked(e,t))radial=.75;
 }else if(['runner','swarm'].includes(e.type)&&d>85)strafe=.55*side;
 const length=Math.max(1,Math.hypot(radial,strafe)),speed=e.speed*(e.slowTime>0?1-e.slow:1);
 let vx=(Math.cos(a)*radial-Math.sin(a)*strafe)*speed/length,vy=(Math.sin(a)*radial+Math.cos(a)*strafe)*speed/length;
 if(boundedArena()){if(e.x<30&&vx<0||e.x>W-30&&vx>0)vx=-vx;if(e.y<30&&vy<0||e.y>H-30&&vy>0)vy=-vy;}
 travel(e,vx,vy,dt);
}
function enemyLead(e,t,dt){
 const old=e.tracking;
 if(old&&old.id===t.id&&dt>0){const v=delta(old,t),blend=Math.min(1,dt*8);e.trackVx=(e.trackVx||0)*(1-blend)+Math.max(-350,Math.min(350,v.x/dt))*blend;e.trackVy=(e.trackVy||0)*(1-blend)+Math.max(-350,Math.min(350,v.y/dt))*blend;}
 else e.trackVx=e.trackVy=0;
 e.tracking={id:t.id,x:t.x,y:t.y};
 const d=delta(e,t),lead=Math.min(.65,Math.hypot(d.x,d.y)/300)*Math.min(.8,.25+wave*.025);
 return Math.atan2(d.y+e.trackVy*lead,d.x+e.trackVx*lead);
}
function enemyAttack(e,t,dt){
 if(!t)return;
 const lead=enemyLead(e,t,dt);
 e.slowTime=Math.max(0,(e.slowTime||0)-dt);
 if(e.beamLeft>0){tickBeam(e,dt);return;}
 if(e.type==='medic'){
  e.attack-=dt;if(e.attack<=0){e.attack=3.5;let healed=0;for(const ally of enemies){if(healed>=3)break;if(ally!==e&&ally.type!=='medic'&&ally.type!=='boss'&&ally.hp>0&&ally.hp<ally.maxHp&&distance(e,ally)<230&&(!inThird()||!Third.blocked(e,ally))){ally.hp=Math.min(ally.maxHp,ally.hp+ally.maxHp*.12);effect({kind:'beam',x:e.x,y:e.y,bx:ally.x,by:ally.y,color:'#49f5cd'});healed++;}}}return;
 }
 if(!['gunner','charger','sentinel','lancer','skirmisher','bomber'].includes(e.type)&&!(wave>=12&&['tank','splitter'].includes(e.type)))return;
 if(e.windup>0){e.windup=Math.max(0,e.windup-dt);if(e.windup<=0){if(e.type==='lancer'){e.beamLeft=.65;e.attack=3/attackPressure();return;}if(e.type==='charger')e.charge=.45;else if(e.type==='bomber'){const count=wave>=15?14:10;for(let n=1;n<count;n++)hostileShot(e,e.aim+n*Math.PI*2/count,185+Math.min(65,wave*2),12+wave*.45);}else if(e.type==='skirmisher'){for(const spread of [-.12,0,.12])hostileShot(e,e.aim+spread,300,8+wave*.4);}else for(let n=0;n<(e.type==='sentinel'?(wave>=15?5:3):wave>=12?3:1);n++)hostileShot(e,e.aim+((e.type==='sentinel'||wave>=12)?(n-(e.type==='sentinel'&&wave>=15?2:1))*.2:0),220+Math.min(90,wave*4),10+wave*.5);e.attack=Math.max(e.type==='bomber'?1.7:.85,(e.type==='bomber'?3.8:e.type==='charger'?2.3:2.0)/(enemyScale.attack*attackPressure()));}}
 ...
 else{e.attack-=dt;if(e.attack<=0){e.aim=['gunner','skirmisher','sentinel'].includes(e.type)?lead:aimAt(e,t);e.windup=e.type==='lancer'?1.15:e.type==='bomber'?1:.65;}}
}
...
function worldToScreen(p,h=0){return inThird()?Third.project(p,cameraFocus(),h):onSphere()?Sphere.project(p,cameraFocus(),h):p;}
function drawRevives(){
 for(const p of activePlayers())if(p.hp>0&&p.invuln>0){const q=worldToScreen(p,12);if(q.visible===false)continue;X.save();X.strokeStyle='#fff4a8';X.lineWidth=3;X.beginPath();X.arc(q.x,q.y,35,0,Math.PI*2);X.stroke();X.restore();}

 for(const p of activePlayers())if(p.hp<=0){
  const point=worldToScreen(p,12);if(point.visible===false)continue;
  X.save();X.strokeStyle=p.downed?p.color:'#687585';X.lineWidth=2;X.globalAlpha=.35;
  if(p.downed){X.beginPath();let started=false;for(let i=0;i<=48;i++){const at=offset(p,i*Math.PI/24,REVIVE_RADIUS),q=worldToScreen(at,2);if(q.visible===false){started=false;continue;}if(started)X.lineTo(q.x,q.y);else X.moveTo(q.x,q.y);started=true;}X.stroke();}
...
function drawMirrorWarnings(){
 ...
 for(const e of enemies.filter(e=>e.mirror&&e.hp>0)){
  if(e.bossCycle?.phase==='warning')ring(e,e.r+16+6*Math.sin(performance.now()/90),'#ffbb70');else if(e.bossCycle?.phase==='recover')ring(e,e.r+12,'#7dffd0');
...
function drawAbilities(){
 ...
 for(const p of activePlayers()){
  if(p.hp<=0)continue;
  if(p.abilities?.blades)for(let n=0;n<2;n++){const at=offset(p,(p.orbitTime||0)+n*Math.PI,58),point=worldToScreen(at,8);if(point.visible===false)continue;X.strokeStyle='#e6c6ff';X.fillStyle='#b487ee';X.lineWidth=2;X.beginPath();X.arc(point.x,point.y,7,0,Math.PI*2);X.fill();X.stroke();}
 }
 X.restore();
}
function shoot(p){
 ...
 for(const [mountIndex,slot] of p.weapons.entries()){
  ...
  const mount=P.mount(p,mountIndex,onSphere()?Sphere:null),origin=mount.tip,angle=origin.angle;
  combat(slot.id,origin,p.id);
  ...
  if(w.reach){
   const end=offset(origin,angle,onSphere()?Math.min(600,w.reach):w.reach),bx=end.x,by=end.y;
   effect({kind:w.arc?'arc':'beam',x:origin.x,y:origin.y,bx,by,angle,r:w.reach,arc:w.arc,color:w.color});
   for(const e of enemies){
    const a=aimAt(origin,e)-angle;
    const hit=w.arc?distance(origin,e)<w.reach+e.r&&Math.abs(Math.atan2(Math.sin(a),Math.cos(a)))<w.arc/2+e.r/Math.max(e.r,distance(origin,e)):distanceToSegment(e.x,e.y,origin.x,origin.y,bx,by)<e.r+5;
    if(hit&&(!inThird()||!Third.blocked(origin,e)))hurt(e,dmg*(1+(p.multi-1)*.15),p.id);
   }
  }else{
   const count=Math.min(16,(w.pellets||1)+p.multi-1);
   for(let n=0;n<count&&shots.length<600&&budget>0;n++,budget--){
    const a=angle+(w.pellets?(n-(count-1)/2)*(w.spread/Math.max(1,count-1)):(Math.random()-.5)*(w.spread||0)+(n-(count-1)/2)*.10);
    const at=offset(origin,a,0);shots.push({x:at.x,y:at.y,vx:Math.cos(at.angle)*w.speed,vy:Math.sin(at.angle)*w.speed,life:w.life*p.range,dmg,weapon:slot.id,owner:p.id,color:w.color,r:w.blast?7:w.burn?9:3,blast:(w.blast||0)*p.blastScale,burn:!!w.burn,pierce:(w.pierce||1)+p.pierce,hitIds:[]});
   }
  }
 }
}
...
function simulate(dt){
 ...
 for(const e of enemies){
  ...
  const t=nearestLiving(e,players);
  ...
  if(t){const a=aimAt(e,t),moving=!e.rival&&!e.ramLeft&&(!e.windup||e.type==='boss')&&!e.beamLeft&&!e.charge&&(!['boss','gunner','sentinel','lancer'].includes(e.type)||distance(e,t)>260);if(e.type!=='boss'&&!e.windup&&!e.beamLeft&&!e.charge)enemySteer(e,t,dt);else if(e.type==='boss'&&moving){travel(e,Math.cos(a)*e.speed*(e.slowTime>0?1-e.slow:1),Math.sin(a)*e.speed*(e.slowTime>0?1-e.slow:1),dt);}if(distance(e,t)<e.r+15&&t.dash<t.dashTime-.15&&!t.invuln){damagePlayer(t,24*dt*enemyScale.damage*waveDamage(),e);e.hit=.08;}}
  e.hit=Math.max(0,e.hit-dt);
 }
 let collisionGrid=window.RRCombatGrid?.create(enemies,Sphere,onSphere()),gridSize=enemies.length;
 for(const s of shots){
  if(enemies.length!==gridSize){collisionGrid=window.RRCombatGrid?.create(enemies,Sphere,onSphere());gridSize=enemies.length;}
  const ax=s.x,ay=s.y;travel(s,s.vx,s.vy,dt,true);s.life-=dt;if(inThird()&&Third.blocked({x:ax,y:ay},s,s.r)){s.life=0;continue;}
  const candidates=collisionGrid?collisionGrid.query({x:ax,y:ay},s,s.r,Math.hypot(s.vx,s.vy)*dt):enemies;
  const hits=candidates.filter(e=>e.hp>0&&!s.hitIds.includes(e.id)&&distanceToSegment(e.x,e.y,ax,ay,s.x,s.y)<e.r+s.r).sort((a,b)=>distance({x:ax,y:ay},a)-distance({x:ax,y:ay},b));
  for(const e of hits){
 ...
 if(fractureLeft>0)return;
 for(const s of enemyShots){const ax=s.x,ay=s.y;travel(s,s.vx,s.vy,dt,true);s.life-=dt;if(inThird()&&Third.blocked({x:ax,y:ay},s,s.r)){s.life=0;continue;}for(const p of activePlayers()){
 ...
 drops=drops.filter(d=>{const t=nearestLiving(d,players);if(!t)return true;const dist=distance(d,t);if(dist<24){collect(d);return false;}if(dist<t.pickup){const v=delta(d,t);travel(d,v.x/dist*320,v.y/dist*320,dt);}return true;});
...
function ui(){
 ...
 const boss=enemies.find(e=>e.type==='boss'&&e.hp>0);$('#bossHud').classList.toggle('hidden',!boss||!running||between);if(boss){$('#bossName').textContent=(boss.mirror?'MIRROR P'+(boss.sourceId+1)+' · '+(boss.bossCycle?boss.bossCycle.kind+' · '+boss.bossCycle.phase.toUpperCase():boss.tactic||'OBSERVING'):boss.rival?'RIVAL · '+(boss.tactic||'OBSERVING'):'WARDEN')+' · W'+wave+(boss.mirror?' · TEAM ×'+boss.advantage.toFixed(2):' · POWER ×'+(boss.power.health*enemyScale.health).toFixed(1))+(boss.mercy?' · LOW-HEALTH RELIEF':'')+(boss.ramWarning>0?' · RAM INCOMING':boss.windup?' · INCOMING':'');$('#bossBar').value=boss.hp;$('#bossBar').max=boss.maxHp;}
...
function update(dt){
 ...
 if(mode==='online'&&isHost&&roomStarted&&netTick>=1/(!running||paused||between?2:links.size>3?15:20)){netTick=0;sendState(checkpointTick>=.5);}
 ...
 for(const p of particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.life-=dt;}particles=particles.filter(p=>p.life>0);uiClock+=dt;if(uiClock>=.1){uiClock=0;ui();}
}
function draw(){
...
function drawFrame(){
 const state={W,H,frontier:openField(),fieldCamera,worldSeed,t:performance.now()/1000,reduced:!!reducedMotion?.matches,players:activePlayers(),enemies,shots,enemyShots,effects,drops,particles,paused,third:inThird(),sphere:onSphere(),focus:cameraFocus()};
 ...
 if(visualTier<2)drawWeaponMounts();
 drawAbilities();drawMirrorWarnings();drawRams();drawDamageNumbers();drawRevives();
 if(openField()&&!inThird())for(const p of activePlayers())if(p.id!==localId&&(p.hp>0||p.downed)&&(p.x<20||p.x>W-20||p.y<20||p.y>H-20)){
 ...
 if(mode==='online'&&!onSphere()&&!inThird()){X.font='bold 11px system-ui';X.textAlign='center';for(const p of activePlayers()){if(p.hp<=0)continue;X.fillStyle=p.color;X.fillText('P'+(p.id+1)+(p.id===localId?' · YOU':''),p.x,p.y+47);}}
 if(inThird()){
  const p=players[localId];if(p){const aim=Third.project(offset(p,p.angle,p.aimDistance||550),cameraFocus(),10);if(aim.visible){X.strokeStyle='#e6fff4';X.lineWidth=2;X.beginPath();X.arc(aim.x,aim.y,7,0,Math.PI*2);X.stroke();}
  X.save();X.translate(W-92,140);X.fillStyle='#06121bdc';X.fillRect(-70,-70,140,140);X.strokeStyle='#6bdcd3';X.strokeRect(-70,-70,140,140);const a=p.cameraAngle??-Math.PI/2,c=Math.cos(a),s=Math.sin(a);for(const q of [...enemies,...activePlayers()]){if(q.hp<=0)continue;const dx=q.x-p.x,dy=q.y-p.y,x=Math.max(-65,Math.min(65,(-dx*s+dy*c)*.06)),y=Math.max(-65,Math.min(65,-(dx*c+dy*s)*.06));X.fillStyle=q.color||'#ff607c';X.fillRect(x-2,y-2,4,4);}X.restore();}
 }
 if(fractureLeft>0)window.RiftVisual?.fracture(X,fractureLeft,!!reducedMotion?.matches,W,H,fractureTarget);
}
function drawWeaponMounts(){
 for(const p of activePlayers())if(p.hp>0)for(const [i,w] of p.weapons.entries()){
 const q=P.mount(p,i,null);X.save();X.translate(q.x,q.y);X.rotate(q.angle);X.fillStyle=P.tiers[w.tier].color;X.strokeStyle=weapons[w.id].color;X.lineWidth=2;
 const wide=['shotgun','rocket','minigun'].includes(w.id)?7:4;
 X.fillRect(-7,-wide,18,wide*2);X.strokeRect(-7,-wide,18,wide*2);
 X.fillStyle=weapons[w.id].color;X.fillRect(8,-2,9,4);
 if(w.id==='shotgun'){X.fillRect(8,-5,9,3);X.fillRect(8,2,9,3);}
 if(['knife','spear'].includes(w.id)){X.beginPath();X.moveTo(5,-5);X.lineTo(23,0);X.lineTo(5,5);X.fill();}
 X.restore();}
}
function drawClassic(){X.fillStyle='#0a1018';X.fillRect(0,0,W,H);X.strokeStyle='#172333';X.lineWidth=1;for(let x=0;x<W;x+=64){X.beginPath();X.moveTo(x,0);X.lineTo(x,H);X.stroke()}for(let y=0;y<H;y+=64){X.beginPath();X.moveTo(0,y);X.lineTo(W,y);X.stroke()}X.strokeStyle='#20364b';X.lineWidth=3;X.strokeRect(18,18,W-36,H-36);drops.forEach(d=>{X.fillStyle=d.kind==='crate'?'#ffc55c':'#68f7c2';X.save();X.translate(d.x,d.y);X.rotate(Math.PI/4);X.fillRect(-5,-5,10,10);if(d.kind==='crate'){X.strokeStyle='#fff';X.strokeRect(-7,-7,14,14);}X.restore();});shots.forEach(s=>{X.fillStyle=s.color||(s.owner?'#54bfff':'#68f7c2');X.shadowBlur=12;X.shadowColor=X.fillStyle;X.beginPath();X.arc(s.x,s.y,s.r||4,0,7);X.fill()});X.shadowBlur=0;enemyShots.forEach(s=>{X.fillStyle='#ff547d';X.beginPath();X.arc(s.x,s.y,s.r,0,Math.PI*2);X.fill();X.strokeStyle='#ffd7e1';X.lineWidth=2;X.stroke();});effects.forEach(f=>{X.save();X.globalAlpha=Math.min(1,f.life*8);X.strokeStyle=f.color;X.lineWidth=5;X.beginPath();if(f.kind==='blast')X.arc(f.x,f.y,f.r,0,Math.PI*2);else if(f.kind==='beam'){X.moveTo(f.x,f.y);X.lineTo(f.bx,f.by);}else{X.moveTo(f.x,f.y);X.arc(f.x,f.y,f.r,f.angle-f.arc/2,f.angle+f.arc/2);X.closePath();}X.stroke();X.restore();});enemies.forEach(e=>{if((e.type==='lancer'||e.beamAttack)&&e.windup>0||e.beamLeft>0){X.save();X.strokeStyle=e.beamLeft>0?'#fff3ff':'#71345d';X.lineWidth=e.beamLeft>0?12:2;X.beginPath();X.moveTo(e.x,e.y);X.lineTo(e.x+Math.cos(e.aim)*1000,e.y+Math.sin(e.aim)*1000);X.stroke();X.restore();}X.save();X.translate(e.x,e.y);if(e.windup>0&&e.type!=='lancer'){X.strokeStyle=enemyTypes[e.type]?.color||'#ff496c';X.lineWidth=3;X.beginPath();X.arc(0,0,e.r+7+e.windup*8,0,Math.PI*2);X.stroke();}if(e.type==='medic'){X.fillStyle='#49f5cd';X.fillRect(-4,-e.r-14,8,12);X.fillRect(-8,-e.r-10,16,4);}X.rotate(performance.now()/900);X.fillStyle=e.hit?'#fff':e.type==='boss'?'#ff6bdd':(enemyTypes[e.type]?.color||'#ff496c');X.strokeStyle=X.fillStyle;X.lineWidth=3;if(e.mirror){X.rotate((e.aim||0)-performance.now()/900);X.beginPath();X.moveTo(28,0);X.lineTo(-17,-18);X.lineTo(-9,0);X.lineTo(-17,18);X.closePath();X.stroke();X.fillRect(2,-3,20,6);}else if(e.type==='boss'){X.beginPath();X.arc(0,0,e.r,0,Math.PI*2);X.stroke();X.rotate(-performance.now()/900);X.fillRect(-18,-18,36,36);for(let n=0;n<6;n++){X.rotate(Math.PI/3);X.fillRect(30,-5,20,10);}}else if(['runner','charger','gunner','skirmisher'].includes(e.type)){X.rotate(e.aim-performance.now()/900);X.beginPath();X.moveTo(e.r+5,0);X.lineTo(-e.r,-e.r);X.lineTo(-e.r,e.r);X.closePath();X.stroke();X.fillRect(-4,-4,8,8);}else if(['tank','sentinel'].includes(e.type)){X.strokeRect(-e.r,-e.r,e.r*2,e.r*2);X.rotate(.7);X.fillRect(-9,-9,18,18)}else{X.beginPath();for(let i=0;i<6;i++){let a=i*Math.PI/3;X.lineTo(Math.cos(a)*e.r,Math.sin(a)*e.r)}X.closePath();X.fill()}X.restore()});players.forEach(p=>{if(p.hp<=0||p.connected===false)return;X.save();X.translate(p.x,p.y);X.rotate(p.angle);X.shadowBlur=20;X.shadowColor=p.color;X.strokeStyle=p.color;X.fillStyle='#0d1721';X.lineWidth=4;X.beginPath();X.arc(0,0,15,0,Math.PI*2);X.fill();X.stroke();X.shadowBlur=0;X.lineWidth=2;if(p.character==='bulwark')X.strokeRect(-19,-19,38,38);else if(p.character==='scout'){X.beginPath();X.moveTo(-12,-19);X.lineTo(21,0);X.lineTo(-12,19);X.stroke();}else if(p.character==='engineer'){for(const side of [-1,1])X.strokeRect(-10,side*21-4,20,8);}else if(p.character==='vanguard'){X.beginPath();X.moveTo(-20,-20);X.lineTo(23,0);X.lineTo(-20,20);X.moveTo(-22,-10);X.lineTo(-22,10);X.stroke();}else if(p.character==='spectre'){X.beginPath();X.moveTo(0,-23);X.lineTo(23,0);X.lineTo(0,23);X.lineTo(-23,0);X.closePath();X.stroke();}X.fillStyle=p.color;X.fillRect(5,-3,14,6);X.restore();X.fillStyle='#1d2937';X.fillRect(p.x-22,p.y+25,44,5);X.fillStyle=p.hp>30?p.color:'#ff496c';X.fillRect(p.x-22,p.y+25,44*Math.max(0,p.hp/p.maxHp),5)});particles.forEach(p=>{X.globalAlpha=Math.max(0,p.life*2);X.fillStyle=p.color;X.fillRect(p.x,p.y,3,3)});X.globalAlpha=1;if(paused){X.fillStyle='#080b12aa';X.fillRect(0,0,W,H);X.fillStyle='#fff';X.font='700 42px system-ui';X.textAlign='center';X.fillText('PAUSED',W/2,H/2)}}

function sendTo(c,m){if(!c?.open)return;const transient=['state','input','ping'].includes(m.t);if(transient&&((c.bufferSize||0)>=3||(c.dataChannel?.bufferedAmount||0)>262144))return;c.send(m);}
...
const livePlayerKeys=['id','x','y','angle','aimDistance','cameraAngle','hp','maxHp','shield','maxShield','invuln','color','connected','lives','downed','reviveProgress','revision','level','xp','materials','pending','ready','speed','dash','dashTime','ramLeft','ramAngle','ramWarning','kickSpeed','kickAngle','enemySlowTime','enemySlow','orbitTime','weapons','abilities'];
const liveEnemyKeys=['id','type','x','y','r','hp','maxHp','color','aim','angle','hit','mirror','sourceId','advantage','power','tactic','windup','beamLeft','beamAttack','slowTime','ramLeft','ramAngle','ramWarning','orbitTime','abilities','novaWarning','weapons','shield','maxShield','charge','mercy','bossCycle'];
function livePose(p,keys){const out={};for(const k of keys){if(p[k]===undefined)continue;const precision=['x','y','vx','vy'].includes(k)?100:['angle','aim','cameraAngle','ramAngle','kickAngle'].includes(k)?10000:0;out[k]=precision&&Number.isFinite(p[k])?Math.round(p[k]*precision)/precision:p[k];}return out;}
const liveBullet=p=>livePose(p,['netId','x','y','vx','vy','r','color']);
...
function restoreState(m,presentation=false){
 ...
 enemyScale=difficultyScale(m.enemyScale?.power??0);visualTier=[1,1.5,2,3,4].includes(m.visualTier)?m.visualTier:0;fractureTarget=[1,1.5,2,3,4].includes(m.fractureTarget)?m.fractureTarget:visualTier;fractureLeft=Number.isFinite(m.fractureLeft)?Math.max(0,Math.min(3,m.fractureLeft)):0;riftBroken=m.riftBroken===true;
 mode='online';roomStarted=true;players=presentation&&m.full===false?m.players.map(p=>({...players.find(old=>old.id===p.id),...p})):m.players;enemies=m.enemies;shots=m.shots;effects=Array.isArray(m.effects)?m.effects:[];if(presentation){if((m.damageSerial||0)<damageSerial){damageNumbers=[];seenDamage.clear();}mergeDamageNumbers(m.damageNumbers);}else damageNumbers=Array.isArray(m.damageNumbers)?m.damageNumbers.slice(-64):[];damageSerial=m.damageSerial||0;drops=Array.isArray(m.drops)?m.drops:[];enemyShots=Array.isArray(m.enemyShots)?m.enemyShots:[];wave=m.wave;score=m.score;spawnLeft=m.spawnLeft;spawnTimer=m.spawnTimer??.6;bossSpawned=!!m.bossSpawned;enemySerial=m.enemySerial||0;running=m.running;paused=m.paused;between=m.between;choices=m.choices||[];
 if(feedback?.setTier(visualTier,presentation&&wasRunning))soundButton();
 ...
 c.on('data',m=>{
  ...
  if(accepting){
   ...
   if(m.t==='input'){
    ...
    const l=Math.max(1,Math.hypot(m.dx,m.dy));link.input={dx:m.dx/l,dy:m.dy/l,angle:m.angle,aimDistance:Number.isFinite(m.aimDistance)?Math.max(60,Math.min(1200,m.aimDistance)):550,fire:m.fire===true,dash:Math.max(0,Math.min(1e9,Math.floor(m.dash)))};
  ...
  if(m.t==='state'){
   ...
   if(between||!running||wave>(profile.reached||0))recordProgress();if(overlay==='lobby')show('none');ui();
  }
 });
 const ended=()=>{
...
$$('[data-action]').forEach(b=>b.onclick=async()=>{
 ...
 if(a==='armory'){startChoice=null;show('armory');armory();}
 ...
 if(a==='armory-done'){const next=startChoice;startChoice=null;if(next==='online'){show('lobby');netStatus('Choose Host or Join.');}else if(next)start(next);else show('menu');}
 if(a==='armory-player'){armoryPlayer=1-armoryPlayer;armory();}
 if(a==='online'){cleanup();startChoice='online';armoryPlayer=0;show('armory');armory();}

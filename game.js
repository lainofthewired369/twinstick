...
function hurt(e,damage,owner,chained=false){if(e.hp<=0)return;const p=players[owner],actual=Math.min(e.hp,damage);if(p&&p.hp>0){p.hp=Math.min(p.maxHp,p.hp+actual*p.leech);if(p.slow){e.slow=p.slow;e.slowTime=1;}}e.hp-=damage;e.hit=.1;if(p?.abilities?.chain&&!chained&&(p.abilityTimers.chain||0)<=0){p.abilityTimers.chain=.45;let from=e;const visited=new Set([e.id]);for(let n=0;n<2;n++){const target=enemies.filter(t=>t.hp>0&&!visited.has(t.id)&&distance(from,t)<170).sort((a,b)=>distance(from,a)-distance(from,b))[0];if(!target)break;visited.add(target.id);effect({kind:'beam',x:from.x,y:from.y,bx:target.x,by:target.y,color:'#83dfff'});hurt(target,damage*.35,owner,true);from=target;}}if(e.hp<=0){if(e.type==='boss')breakRift();if(p&&p.hp>0)p.hp=Math.min(p.maxHp,p.hp+p.killHeal);if(e.type==='splitter'&&enemies.length<140)for(let n=0;n<3;n++)spawnVariant('swarm',e.x+Math.cos(n*2.1)*22,e.y+Math.sin(n*2.1)*22);score+=e.type==='boss'?500:e.type==='tank'?50:20;drops.push({x:Math.max(20,Math.min(W-20,e.x)),y:Math.max(20,Math.min(H-20,e.y)),kind:'material',value:e.type==='boss'?35:e.type==='tank'?6:3});if(e.type==='boss'||(e.type==='tank'&&Math.random()<.15))drops.push({x:e.x,y:e.y,kind:'crate',weapon:Object.keys(weapons)[Math.floor(Math.random()*10)]});burst(e.x,e.y,e.type==='tank'?'#ffc55c':'#ff496c',14);}}
...
function bossAttack(e,target,dt){
 ...
 if(e.windup>0){e.windup=Math.max(0,e.windup-dt);if(e.windup>0)return;
  if(e.beamAttack){e.beamLeft=.85;e.beamAttack=false;e.pattern++;e.attack=1.2/latePressure();return;}
...
function latePressure(){return 1+Math.max(0,wave-7)*.1;}
function tickBeam(e,dt){
 e.beamLeft=Math.max(0,(e.beamLeft||0)-dt);
 const length=onSphere()?600:1000;
 for(const p of activePlayers()){const d=delta(e,p),along=d.x*Math.cos(e.aim)+d.y*Math.sin(e.aim),across=Math.abs(d.y*Math.cos(e.aim)-d.x*Math.sin(e.aim));if(p.hp>0&&along>0&&along<length&&across<21&&!p.invuln&&p.dash<p.dashTime-.15)damagePlayer(p,(40+wave*2)*enemyScale.damage*dt);}
}
function spawnVariant(type,x,y){const v=enemyTypes[type];enemies.push({id:++enemySerial,type,x,y,r:v.r,hp:(v.hp+wave*(type==='swarm'?2:7))*enemyScale.health*latePressure(),speed:Math.min(270,v.speed+wave*2),hit:0,burn:0,burnDamage:0,attack:(1+Math.random())/enemyScale.attack,windup:0,aim:0,charge:0,slow:0,slowTime:0});}
...
function enemyAttack(e,t,dt){
 ...
 if(e.windup>0){e.windup=Math.max(0,e.windup-dt);if(e.windup<=0){if(e.type==='lancer'){e.beamLeft=.65;e.attack=3/latePressure();return;}if(e.type==='charger')e.charge=.45;else for(let n=0;n<(e.type==='sentinel'?(wave>=15?5:3):wave>=12?3:1);n++)hostileShot(e,e.aim+((e.type==='sentinel'||wave>=12)?(n-(e.type==='sentinel'&&wave>=15?2:1))*.2:0),220+Math.min(90,wave*4),10+wave*.5);e.attack=(e.type==='charger'?2.3:2.0)/(enemyScale.attack*latePressure());}}
...
function tickAbilities(p,dt){
 ...
 if(p.abilities?.repair&&(timers.repair||0)<=0){timers.repair=4;for(const ally of activePlayers())if(ally.hp>0&&distance(p,ally)<=160)ally.hp=Math.min(ally.maxHp,ally.hp+8);effect({kind:'blast',x:p.x,y:p.y,r:160,color:'#7dffd0'});}
...
function simulate(dt){
 ...
 if(fractureLeft>0)return;
 for(const s of enemyShots){const ax=s.x,ay=s.y;travel(s,s.vx,s.vy,dt,true);s.life-=dt;for(const p of activePlayers()){if(p.hp<=0||distanceToSegment(p.x,p.y,ax,ay,s.x,s.y)>s.r+15)continue;s.life=0;if(!p.invuln&&p.dash<p.dashTime-.15){damagePlayer(p,s.damage);p.invuln=.6;burst(p.x,p.y,'#ff496c',8);}break;}}
...
function update(dt){
 ...
 if(mode==='online'){
  ...
  if(isHost){
   ...
   if(!roomStarted&&lobbyTick>=1){lobbyTick=0;send({t:'ping',v:12});}
  }else if(conn?.open){
...
function sendState(full=true){
 ...
 roster=[...seats.values()];
 const state={t:'state',v:12,full,...roomMeta(),migrating,enemies,shots:full?shots:shots.map(({x,y,vx,vy,r,color})=>({x,y,vx,vy,r,color})),effects,drops,enemyShots,visualTier,fractureLeft,riftBroken,fractureTarget,enemyScale,wave,score,spawnLeft,spawnTimer,bossSpawned,enemySerial,running,paused,between,choices};
...
function broadcastLobby(){
 if(!isHost)return;roster=[...seats.values()];renderLobby();send({t:'lobby',v:12,...roomMeta()});saveRoom();
...
async function roomId(password){
 const hash=await crypto.subtle.digest('SHA-256',new TextEncoder().encode('rift-runners-v12:'+password));
 return 'rr12-'+Array.from(new Uint8Array(hash),b=>b.toString(16).padStart(2,'0')).join('');
}
function redirect(c){
 const reply=()=>{sendTo(c,{t:'redirect',v:12,peerId:hostPeerId});setTimeout(()=>c.close?.(),500);};
...
function attachPeer(p,token){
 ...
 p.on('error',e=>{
  ...
  if(['peer-unavailable','webrtc'].includes(e.type)&&!isHost)return;
...
async function connect(host){
 cleanup();const token=session;isHost=host;localId=host?0:-1;mode='online';show('lobby');
 ...
 try{
  ...
  const p=new Peer(host?id:undefined,{debug:0});peer=p;attachPeer(p,token);
  p.on('open',()=>{
   if(token!==session)return;clearTimeout(netTimer);
   if(host){hostPeerId=p.id;roomOpen=true;seats.set(0,{id:0,token:roomToken,config:P.config(configs[0]),peerId:p.id,connected:true});broadcastLobby();}
   else {
...
function tryJoin(){
 ...
 if(recovering){
  ...
  if(!hostPeerId){netStatus('Room unreachable. Check the password and retry JOIN ROOM.');return;}
 }
 ...
 netTimer=setTimeout(()=>{if(!isHost)retryJoin();},recovering?8000:6000);
}
...
function claimDirectory(token,attempt=0){
 ...
 const b=new Peer(roomKey,{debug:0});beacon=b;
...
function wire(c,token){
 ...
 if(accepting){
  ...
  handshakeTimer=setTimeout(()=>c.close?.(),20000);
 }else conn=c;
 c.on('open',()=>{if(token!==session)return;lastPacket=performance.now();if(!accepting)sendTo(c,{t:'ready',v:12,token:roomToken,config:P.config(configs[0])});});
 c.on('data',m=>{
  if(token!==session||!m||typeof m!=='object')return;
  if(accepting){
   if(!isHost)return;
   if(m.t==='ready'&&!link){
    if(m.v!==12||typeof m.token!=='string'||m.token.length<16||m.token.length>128){rejectConnection(c,'Refresh to v1.10 and join again.');return;}
    ...
    sendTo(c,{t:'welcome',v:12,id,...roomMeta()});broadcastLobby();sendState();return;
   }
 ...
 c.on('close',ended);c.on('error',ended);
}
